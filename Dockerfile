...
RUN apk update && apk --no-cache add --virtual .build-deps gcc libc-dev make openssl-dev pcre-dev \
    ...
    autoconf libtool go clang git wget curl patch cmake g++ yajl-dev pkgconf libcurl redis
...
RUN apk add --no-cache tzdata pcre libgcc yajl libstdc++ libmaxminddb-dev lmdb-dev libxml2-dev curl-dev && \
    addgroup -S nginx && \
